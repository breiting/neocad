...
#include <neocad/editor/Mode.hpp> // Not strictly needed in header, but for context
#include <vector>
...
class SketchCurveTool : public ITool {
   ...
   private:
    ...
    std::vector<nc::domain::Entity> m_Points; ///< List of placed point entities for the current sketch.

    nc::domain::Entity m_PreviewPoint = domain::INVALID_ENTITY; ///< Entity for the dynamically updated preview point.
    nc::domain::Entity m_LastPreviewLine = domain::INVALID_ENTITY; ///< Entity for the dynamically updated preview line.
